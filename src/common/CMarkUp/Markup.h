...
public:
	...
	//[CMARKUPDEV
	bool FindPrevElem( MCD_CSTR szName=NULL );
	bool FindPrevChildElem( MCD_CSTR szName=NULL );
	MCD_STR FindGetData( MCD_CSTR szName );
	int GetElemIndex() const { return m_iPos; };
	void GotoElemIndex( int i ) { x_SetPos(x_GetParent(i),i,0); };
	int GetChildElemIndex() const { return m_iPosChild; };
	void GotoChildElemIndex( int i ) { int iParent=x_GetParent(i); x_SetPos(x_GetParent(iParent),iParent,i); };
	int GetParentElemIndex() const { return m_iPosParent; };
	void GotoParentElemIndex( int i ) { x_SetPos(i,0,0); };
	bool GetOffsets( int* pnStart, int* pnLen, int* pnInStart=NULL, int* pnInLen=NULL ) const;
	bool GetAttribOffsets( MCD_CSTR szAttrib, int* pnStart, int* pnLen, int* pnInStart=NULL, int* pnInLen=NULL ) const;
	MCD_STR GetElemPath() const;
	MCD_STR GetChildElemPath() const { return x_GetPath(m_iPosChild); };
	MCD_STR GetParentElemPath() const;
	int GetElemLevel();
	int GetElemFlags() const;
	void SetElemFlags( int nFlags );
	int GetDocElemCount();
	MCD_STR GetDocFormatted( int nFormatFlags = 0 );
	//]CMARKUPDEV
	...
	//[CMARKUPDEV
	bool RemoveAttrib( MCD_CSTR szAttrib ) { return x_RemoveAttrib(m_iPos,szAttrib); };
	bool RemoveChildAttrib( MCD_CSTR szAttrib ) { return x_RemoveAttrib(m_iPosChild,szAttrib); };
	bool HasAttrib( MCD_CSTR szAttrib, MCD_STR* pstrValue = NULL ) const { return x_HasAttrib(m_iPos,szAttrib,pstrValue); };
	bool HasChildAttrib( MCD_CSTR szAttrib, MCD_STR* pstrValue = NULL ) const { return x_HasAttrib(m_iPosChild,szAttrib,pstrValue); };
	...
	bool Close();
	bool Flush();
	MCD_INTFILEOFFSET GetOpenFileSize();
	MCD_INTFILEOFFSET GetOpenFileOffset();
	//]CMARKUPDEV
	...
	//[CMARKUPDEV
	static MCD_STR EncodeBase64( const unsigned char* pBuffer, int nBufferLen );
	static int DecodeBase64( const MCD_STR& strBase64, unsigned char* pBuffer, int nBufferLen );
	//]CMARKUPDEV
...
#define MARKUP_SETDEBUGSTATE
	...
	//[CMARKUPDEV
	int x_FindPrevElem( int iPosParent, int iPos, MCD_PCSZ pName );
	bool x_RemoveAttrib( int iPos, MCD_PCSZ pAttrib );
	bool x_HasAttrib( int iPos, MCD_PCSZ pAttrib, MCD_STR* pstrValue ) const;
	bool x_FindNodeReader( int nType, MCD_PCSZ pszPath );
	MCD_STR x_GetMarkupReader( bool bSubDoc );
	//]CMARKUPDEV
